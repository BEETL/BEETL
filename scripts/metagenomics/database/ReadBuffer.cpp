/**
 	...
 	**/
 	...
 	{
 	assert((numRead%readSize_)==0); // ensure whole number of seqs are read, else a problem
 	lastIter_=true;
 	...
 	assert(read( fdPtr_, seqPtr_, sizeof(LetterCountType)*maxEntry_)==sizeof(LetterCountType)*maxEntry_);
