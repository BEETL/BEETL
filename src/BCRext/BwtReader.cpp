...
BwtReaderBase::BwtReaderBase( const std::string& fileName ) : 
...
uint BwtReaderBase::readAndCount( LetterCount& c )
{ // will call the relevant virtual function
  return readAndCount( c, maxLetterCountType );
} // ~readAndCount

uint BwtReaderBase::readAndSend( BwtWriterBase& writer )
{ // will call the relevant virtual function
...
uint BwtReaderASCII::readAndCount( LetterCount& c, const LetterCountType numChars )
{ 
#ifdef DEBUG
...
uint BwtReaderASCII::readAndSend(BwtWriterBase& writer, const int numChars) {
#ifdef DEBUG
...
#define XXX 1
#ifdef XXX
    uint charsLeft = numRead;

    for (uint counter = 0; counter < charsLeft; counter++) {
...
uint BwtReaderRunLength::readAndCount( LetterCount& c, const LetterCountType numChars ) 
{ 
#ifdef DEBUG
...
uint BwtReaderRunLength::readAndSend(BwtWriterBase& writer, const int numChars )
{ 
#ifdef DEBUG
...
BwtReaderHuffman::BwtReaderHuffman( const std::string& fileName ):
...

{
    ...
    {
      tokenTable_[i]=0xFF;
      for (uint j(0);j<numSingleCodes;j++)
      {
	...
	if ((i&codeMask)==singleCharCode[j])
	{
      ...
      for (uint j(0);j<numDoubleCodes;j++)
      {
...
uint BwtReaderHuffman::readAndCount( LetterCount& c, const LetterCountType numChars ) 
{ 
...
uint BwtReaderHuffman::readAndSend(BwtWriterBase& writer, const int numChars )
{ 
    if (numChars == 0) {return numChars;} // exit directy
...
bool BwtReaderHuffman::getRun(void)
{
    ...
    while (bitsUsed_ > 32) { // as long as we have some more bits than 1 int uses
        ...
        if ((codeNum & 0x1) == 0) // single code
        {
            ...
            if (codeNum == finalCharCode) {nearlyFinished_ = true; break;}
...
uint BwtReaderHuffman::getNum(int& i) {
    uint n(soFar_.ui & 0xF); // only last 4 bit -> 4 bit encodig the number
