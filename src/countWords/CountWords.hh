...
#define COUNTWORDS_HH_ID "@(#) $Id: CountWords.hh,v 1.4 2011/12/20 14:33:24 tjakobi Exp $"
...
// code below used to include countWords as subprogram into main executable
// ./beetl command flags
...
struct RangeState
{
  ...
  void getSeq( string& word )
  {
    ...
    if (wordLast_[0]!=notInAlphabet)
    {
      ...
      for (uint i(0);i<strlen(wordLast_);i++)
  ...
  void addNum( LetterCountType num )
  {
    ...
    const uint toWrite((fragBuf_+fragBufSize)-pFrag);
#ifdef DEBUG
