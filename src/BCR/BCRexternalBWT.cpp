...
int BCRexternalBWT::buildBCR(char const * file1, char const * fileOut)
{
	...
	*/
	...
	for (int t = lengthRead-2 ; t > 0; t--) {
		std::cerr << "\n"<< "j= "<< lengthRead - t <<" - symbols in position " << t << "\n";
	...
	std::cerr << "\n"<< "j= "<< lengthRead <<" - symbols in position " << 0 << "\n";
...
void BCRexternalBWT::InsertFirstsymbols(uchar const * newSymb)
{
	...
	for (dataTypeNSeq j = 0 ; j < nText; j++) {
		tableOcc[0][alpha[(int)newSymb[j]]]++;       //counting the number of occurrences in BWT of the $-pile
		/*
		if (newSymb[j] < 50) {
...
void BCRexternalBWT::InsertNsymbols(uchar const * newSymb, dataTypelenSeq posSymb)
{
	...
	while (j < nText) {			
		...
		while ((k< nText) && (vectTriple[k].pileN == currentPile)) {
			...
			for (dataTypedimAlpha g = 0 ; g < currentPile; g++) {  //I have to count in each pile g= 0... (currentPile-1)-pile
				vectTriple[k].posN = vectTriple[k].posN + tableOcc[g][alpha[(int)foundSymbol]];
				//std::cerr << "--New posN[k]=" << (int)posN[k] << " tableOcc[g][alpha[(int)symbol]] " << tableOcc[g][alpha[(int)symbol]] <<std::endl;
				if (verboseEncode == 1) {				
					std::cerr << "g= " << (int)g << " symbol= " << (int)foundSymbol << " alpha[symbol]= "<< (int)alpha[(int)foundSymbol] <<std::endl;
					std::cerr << "Add New posN[k]=" << vectTriple[k].posN << " tableOcc[g][alpha[(int)symbol]] " << tableOcc[g][alpha[(int)foundSymbol]] <<std::endl;
				}
...
void BCRexternalBWT::storeBWT(uchar const * newSymb) {
	...
	while (j < nText) {	
		...
		if (InFileBWT==NULL) {
				// NO, abort program
			std::cerr << "In BWT file " << (int)j <<": Error opening " << std::endl;
			exit (EXIT_FAILURE);
		}
		...
		if (OutFileBWT==NULL) {
				// NO, abort program
				std::cerr << "Out BWT file " << (int)j <<": Error opening " << std::endl;
				exit (EXIT_FAILURE);
		}
		...
		while ((k< nText) && (vectTriple[k].pileN == currentPile)) {
			...
			while (toRead > 0) {            //((numchar!=0) && (toRead > 0)) {
				...
				if ((numchar == 0) && (toRead > 0)) {  //it means that we have read 0 character, but there are still toRead characters to read
					// NO, abort program
					std::cerr << "storeBWT: sequence number" << (int)k << " read 0 character, but there are still " << toRead << " characters to read  " << std::endl;
					exit (EXIT_FAILURE);
				}
			...
			if (toRead==0) {
				...
				tableOcc[currentPile][alpha[(int)newSymb[vectTriple[k].seqN]]]++;       //update the number of occurrences in BWT of the pileN[k]
